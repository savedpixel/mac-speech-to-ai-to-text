...
final class MenuBarController: NSObject, NSMenuDelegate {
    ...
    private func setupStatusItem() {
        if let button = statusItem.button {
            button.image = NSImage(systemSymbolName: "mic.circle", accessibilityDescription: "Mac Speech to AI to Text")
        }
        rebuildMenu()
    }
    ...
    private func updateIcon() {
        if settings.micDisconnected && pipelineCoordinator.state == .idle {
            statusItem.button?.image = NSImage(
                ...
                accessibilityDescription: "Mac Speech to AI to Text — Mic Disconnected"
            )
            return
        }
        ...
        statusItem.button?.image = NSImage(
            ...
            accessibilityDescription: "Mac Speech to AI to Text"
        )
    }
    ...
    private func rebuildMenu() {
        ...
        let showItem = NSMenuItem(
            title: "Show Mac Speech to AI to Text",
        ...
        let quitItem = NSMenuItem(
            title: "Quit Mac Speech to AI to Text",
    ...
    @objc private func showMainWindow() {
        ...
        if let window = NSApp.windows.first(where: { $0.identifier?.rawValue == "main" || $0.title == "Mac Speech to AI to Text" }) {
