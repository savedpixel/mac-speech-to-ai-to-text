...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Register macOS Services provider for right-click → Services → "Start Mac Speech to AI to Text"
        ...
        DispatchQueue.main.async { [weak self] in
            if let window = NSApplication.shared.windows.first(where: { $0.title == "Mac Speech to AI to Text" }) {
